...
if ( ! class_exists( 'Interium_Woo_Module' ) ) {

	/**
	 * Define Interium_Woo_Module class
	 */
	class Interium_Woo_Module extends Interium_Module_Base {
		...
		public function filters() {
			...
			add_filter( 'interium-theme/assets-depends/script', array( $this, 'assets_depends_script' ) );

			add_filter( 'interium-theme/customizer/options', array( $this, 'customizer_options' ) );

			add_filter( 'cx_customizer/core_sections', array(  $this, 'interium_customizer_core_sections' ) );
		}
		...
		*/
		...
		public function interium_customizer_core_sections( $sections ) {
		...
		public function customizer_options( $options ) {
			...
			$new_options = array(
				'woocommerce_accent_color' => array(
					'title'    => esc_html__( 'WooCommerce Accent color', 'interium' ),
				...
				'woocommerce_settings' => array(
					'title'           => esc_html__( 'WooCommerce Options', 'interium' ),
					'priority'        => 200,
					'type'            => 'panel',
				),
				'single_product_layout' => array(
					'title'    => esc_html__( 'Single product layout', 'interium' ),
					'panel'    => 'woocommerce_settings',
					'priority' => 1,
					'type'     => 'section',
				),
				'single_product_container_type' => array(
					'title'   => esc_html__( 'Single product container type', 'interium' ),
		...
		public function assets_depends_script( $scripts_depends ) {
			array_push( $scripts_depends, 'interium-woo-module-script' );
