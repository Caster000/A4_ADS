 ...
 * @return array
 */
function interium_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = interium_theme()->customizer->get_value( 'related_posts_visible' );
	...
	$post_terms  = array();
	$post_number = interium_theme()->customizer->get_value( 'related_posts_count' );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = interium_theme()->customizer->get_value( $setting_value );
	}
	...
	if ( $holder_view_dir ) {
		...
		foreach ( $posts as $post ) {
			...
			$image = ( $settings['image_visible'] ) ? interium_post_thumbnail( 'interium-thumb-s', array( 'echo' => false ) ) : '';
			...
			$author = ( $settings['author_visible'] ) ? interium_posted_by( array( 'echo' => false ) ) : '';

			$date = ( $settings['date_visible'] ) ? interium_posted_on( array( 'echo' => false ) ) : '';
